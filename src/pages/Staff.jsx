...
export default function Staff({lang}){
  ...
  function submit(){
    ...
    setSelected([]); setComments(''); setShowError(false);
  }
  ...
  return (
    <div className="min-h-screen p-6 max-w-3xl mx-auto">
      ...
      <div className="card">
        <input className="input mb-4" placeholder={t.namePlaceholder} value={name} onChange={e=>setName(e.target.value)} />
        <select className="input mb-4" value={role} onChange={e=>setRole(e.target.value)}>
          <option value="">{t.rolePlaceholder}</option>
          {t.roleOptions.map(r => <option key={r} value={r}>{r}</option>)}
        </select>
        <div className="mb-4">
          <div className="text-center text-2xl font-semibold my-4">{monthLabel}</div>
          <Calendar date={current} selected={selected} setSelected={setSelected} weekdayLabels={t.weekdays} allowedWeekdays={[3,4,5]} />
        </div>
        <textarea className="input mb-4 h-24 resize-none" placeholder={t.commentsPlaceholder} value={comments} onChange={e=>setComments(e.target.value)} />
...
<button className="btn w-full py-4 text-lg" onClick={submit}>{t.send}</button>
        {closed && showError && <div className="mt-3 text-red-400">{t.closed}</div>}
      </div>
    </div>
  )
}
